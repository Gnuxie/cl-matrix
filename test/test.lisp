...
(define-test pagination-chat
  ...
  (let ((token-before (nth-value 1 (cl-matrix:with-account (*user-one*) (cl-matrix:account-sync)))))
    (cl-matrix:with-account (*user-one*)
      (true (member *pagination-chat* (cl-matrix:user-joined-rooms) :test #'string=)))
    (format t "room: ~a~%" *pagination-chat*)
    (format t "collecting events before...~%")
    (cl-matrix:with-account (*user-one*)
      (let ((events-before (cl-matrix:n-messages *pagination-chat* token-before "b" 100)))
        ...
        (let* ((new-events (reverse (cl-matrix:all-messages *pagination-chat* token-before "f")))
          ...
          (is string= "29" (jsown:filter (first (remove-if-not #'(lambda (x) (string= "m.room.message" x))
                                                               events-after-forwards :key #'cl-matrix:event-type))
                                         "content" "body"))

          (is string= "0" (jsown:filter (car (last (remove-if-not #'(lambda (x) (string= "m.room.message" x))
                                                                  events-after-forwards :key #'cl-matrix:event-type)))
                                        "content" "body")))))))
